...
////* this file is to create a task from a calendar. In this file if you want to relate this task to a particular module
//* such as Leads, Contacts, Accounts or Opportunities then you have to select module name from the dropdown and then 
//* from the next dropdown you have to select a particular module item. Then this task gets related to that particular item


class CreateTaskCalendar extends Component {
    ...
    createTask() {
        ...
        if (proceed) {
            ...
            let url = globals.home_url + "/ustore" + "?token_id=" + this.props.token + "&module_name=" + globals.tasks +
                "&name=" + this.state.title + "&status=" + this.state.status + "&reminder_c=" + this.state.reminder +
    ...
    setReminder(response) {
        this.notif.scheduleNotification(response, globals.tasks, globals.type_calendar)
    }
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(CreateTaskCalendar);