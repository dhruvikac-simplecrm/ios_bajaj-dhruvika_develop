...
//* this is a separate file to update task details for account

class UpdateTaskAccount extends Component {
    ...
    constructor(props) {
        ...
        this.state.adata = this.props.data;//get the account data from the detail view
        console.log('task id');
        console.log(this.state.adata.id);
    ...
    updateTask(){
        ...
        if(proceed){
            ...
            let url = globals.home_url+"/uupdate"+"?token_id="+ this.props.token+ "&id=" + this.state.adata.id + "&module_name="+ globals.tasks + 
            ...
            if (response.success === true) {
                ...
                Navigation.startSingleScreenApp({
                    screen: {
                        ...
                        passProps: {id: response.data.result.id,type: 'create',module: 'Tasks',account_id: this.state.parent_id, parent_name: this.state.parent_name}, // simple serializable object that will pass as props to all top screens (optional)
                    animationType: 'slide-horizontal'
                })
    ...
    setReminder(response) {
        this.notif.updateScheduledNotification(response, globals.tasks, globals.accounts)
    }
    ...
    relateTask(id){
        ...
        "&module_id=" + this.state.parent_id+"&link_field_name=tasks" 
        ...
        .then((response) => {
            ...
            if (response.success === true) {
                ...
                // AlertIOS.alert(globals.app_name, globals.app_messages.task_create_message,
                ...
                //                 },
                //                     passProps: {id: id,type: 'create',module: 'Tasks',account_id: this.state.account_data.id, parent_name: parent_name}, // simple serializable object that will pass as props to all top screens (optional)
                //                 animationType: 'slide-horizontal'
                //             })
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(UpdateTaskAccount);