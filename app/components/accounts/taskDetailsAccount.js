...
//* this file is a task detail file. Get the task details from the CRM. This task is also related to the Account
class TaskDetailsAccount extends Component {
        ...
        if (event.type == 'NavBarButtonPress') {
            ...
            if (event.id == 'edit') { // this is the same id field from the static navigatorButtons definition
                ...
                this.props.navigator.push({
                    screen: 'app.UpdateTaskAccount',
                    title: 'Edit Task',
                    subtitle: undefined, // navigation bar subtitle of the pushed screen (optional)
                    passProps: { data: this.state.task_data, account_id:  this.props.account_id, account_name: parent_name }, // Object that will be passed as props to the pushed screen (optional)
    ...
    delete(id){
        ...
        let url = globals.home_url + "/udestroy" + "?token_id=" + this.props.token + "&id=" + id + "&module_name=" + globals.tasks
        + "&url="+this.props.url;
        console.log(url);
    ...
    componentDidMount = () => {
        this.getDetails();//get the tasks details
    };
    ...
    getDetails() {
        ...
        fetch(url, {
            ...
            .then((response) => {
                ...
                if (response.success === true) {
                    // your code here
                    this.setState({ task_data: response.data.result });//save the task data in the state array variable
                    this.setReminderValue(this.state.task_data.reminder_c);//format the reminder value
                    this.setState({
                        date_formatted: globals.formatUtcDateAndTimeToLocal(this.state.task_data.date_due)//format the task due date here
                    });
                } else if (response.data.number === globals.apiErrorCodes.ECODE_TOKEN_EXPIRED) {
                    // this.setState({ loading: false });
                    // Alert.alert(
                    ...
                    <View style={{ padding: 5 }}>
                        ...
                        {/* <Text style={{ fontSize: 15, color: '#0db289', paddingTop: 0, paddingLeft: 10, paddingBottom: 10, backgroundColor: '#fafafa' }}>+92 20982393753</Text>
                            ...
                            <View style={{ flex: 1 }} >
                                ...
                                <Text style={{ fontSize: 15, color: 'black', paddingTop: 0, paddingLeft: 10, paddingBottom: 10, backgroundColor: '#fafafa' }}>{this.state.task_data.status}</Text>
                        ...
                        <Label style={{ fontSize: 12.5, color: 'grey', padding: 10, backgroundColor: '#fafafa' }}>Topic</Label>


                        <View style={{ flex: 1, flexDirection: 'row' }} >
                            <View style={{ flex: 1 }} >
                                <Text style={{ fontSize: 15, color: 'black', paddingTop: 0, paddingLeft: 10, paddingBottom: 10, backgroundColor: '#fafafa' }}>{this.state.task_data.name}</Text>
...
{/* 
                        ...
                        <Text style={{ fontSize: 15, color: 'black', paddingTop: 0, paddingLeft: 10, paddingBottom: 10, backgroundColor: '#fafafa' }}>{this.state.reminder_c}</Text>
                        <Item></Item> */}
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(TaskDetailsAccount);