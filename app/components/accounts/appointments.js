...
import { Container,Card, Content, ListItem, Body, Text } from 'native-base';
...
//* this file shows a list of appointments which are related to a specific account
//* appointments are consist of activity records such as
//*  calls, meetings and tasks which are planned, In progress


class AppointmentsAccount extends Component {
    constructor(props) {
        super(props);
    ...
    //* this function will get the list of all the appointments
    getAppointments(){
        ...
        "&module_id=" + this.state.account_data.id + "&status_array=" +globals.APPOINTMENT_STATUS
        + "&url="+this.props.url;
    

        console.log('accounts appointments url: '+url);
        ...
        .then((response) => {
            ...
            if (response.success === true) {
                ...
                if (typeof response.data.relationship_list !== 'undefined') {
                    // your code here
                    //* storing all the records in a state array variable datas
                    console.log('loading can be seen');
    ...
    //* appointments contains mix data of calls, meetings or tasks so 
    //* you have to decide where to send user on click
    //* this function will take user to the record they want to see the details of.
    goDetails (id, module){
...
//* this is the UI part which contains list item and a loader
    render() {
        return (
                                        ...
                                        <View>
                                                ...
                                                <View style={{ flex: 0.95 }}>
                                                {
                                                    ...
                                                    {item.date_start != '' || item.date_due != '' ?
                                                            ( item.date_start != '' && <View ><Text style={{ fontSize: 14, color: globals.colors.grey, fontStyle: 'italic' }} >{
                                                                globals.formatUtcDateAndTimeToLocal(item.date_start)
                                                            }</Text></View>
                                                           || item.date_due != '' && <View ><Text style={{ fontSize: 14, color: globals.colors.grey, fontStyle: 'italic' }} >{
                                                                globals.formatUtcDateAndTimeToLocal(item.date_due)
                                                            }</Text></View>
                                                            ) :
                                                            (
                                                                <View ><Text style={{ fontSize: 14, color: globals.colors.grey, fontStyle: 'italic' }} >{globals.missing_texts.date_missing}</Text></View>
                                                            )
                                                        }

                                                        
                                                        {/* {item.date_entered != '' ?
                                                            (<View ><Text style={{ fontSize: 14, color: globals.colors.grey, fontStyle: 'italic' }} >{globals.formatUtcDateAndTimeToLocal(item.date_start)}</Text></View>
                                                            ) :
                                                            (
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(AppointmentsAccount);