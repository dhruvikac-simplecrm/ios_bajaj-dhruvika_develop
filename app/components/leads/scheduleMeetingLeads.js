...
class ScheduleMeetingLeads extends Component {
    ...
    scheduleMeeting(){
        ...
        // "&reminder_time="+ this.state.reminder +
         ...
         "&assigned_user_name="+this.state.adata.assigned_user_name + "&location="+ this.state.location +
        ...
        .then((response) => {
            ...
            if (response.success === true) {
                ...
                this.relateMeeting(response.data.result.id, this.state.parent_name);
                ...
                apiCallForToken.getToken(this.props).then(token => {
                    ...
                    //create the meeting, after session generated
                    this.scheduleMeeting()
                }).catch(error => {
    ...
    setReminder(response) {
        this.notif.scheduleNotification(response, globals.meetings, globals.leads)
    }


    //relate this meeting to the lead
    relateMeeting(id, parent_name){
            ...
            if (response.success === true) {
                ...
                Navigation.startSingleScreenApp({
                    screen: {
                        ...
                        passProps: {id: id,type: 'create',module: 'Meetings',lead_id: this.state.parent_id, parent_name: parent_name}, // simple serializable object that will pass as props to all top screens (optional)
                    animationType: 'slide-horizontal'
                })
            }else if (response.data.number === globals.apiErrorCodes.ECODE_TOKEN_EXPIRED) {
                // Alert.alert(
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(ScheduleMeetingLeads);