...
class UpdateCallLead extends Component {
    ...
    logCall(){
        ...
        if(proceed){
            ...
            let url = globals.home_url + "/uupdate" + "?token_id=" + this.props.token + "&id=" + this.state.adata.id + "&module_name=" + globals.calls +
                ...
                + "&parent_type=" + this.state.module_name
                + "&url=" + this.props.url;
        console.log(url);
            ...
            if (response.success === true) {
                ...
                Navigation.startSingleScreenApp({
                    screen: {
                        ...
                        passProps: {id: response.data.result.id,type: 'create',module: 'Calls',lead_id: this.state.parent_id, parent_name: this.state.parent_name}, // simple serializable object that will pass as props to all top screens (optional)
                    animationType: 'slide-horizontal'
                });
...
/**
    ...
    setReminder(response) {
        this.notif.updateScheduledNotification(response, globals.calls, globals.leads)
    }


    relateCall(id){
        let url = globals.home_url+"/setRelationship"+"?token_id="+ this.props.token +"&module_name="+ globals.leads + 
        "&module_id=" + this.state.parent_id+"&link_field_name=calls" + "&related_ids="+ id+"&delete=0"
        + "&url="+this.props.url;
        ...
        .then((response) => {
            ...
            if (response.success === true) {
                ...
                //     },
                //         passProps: {id: id,type: 'create',module: 'Calls',lead_id: this.state.adata.id, parent_name: parent_name}, // simple serializable object that will pass as props to all top screens (optional)
                //     animationType: 'slide-horizontal'
                // })
            }else if (response.data.number === globals.apiErrorCodes.ECODE_TOKEN_EXPIRED) {
                // Alert.alert(
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(UpdateCallLead);