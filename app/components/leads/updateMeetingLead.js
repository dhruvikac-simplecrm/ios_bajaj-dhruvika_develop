...
class UpdateMeetingLead extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            relate_to: '',
            parent_name: this.props.parent_name,//this.props.data.name,
            module_name: 'Leads',
        ...
        this.state.parent_id = this.props.lead_id;
        console.log('meeting id in schedule meeting: '+ this.state.adata.id);
    ...
    updateMeeting(){
        ...
        if(proceed){
            ...
            let url = globals.home_url+"/uupdate"+"?token_id="+ this.props.token +"&id="+ this.state.adata.id + "&module_name="+ globals.meetings + 
            ...
            if (response.success === true) {
                ...
                Navigation.startSingleScreenApp({
                    screen: {
                        ...
                        passProps: {id: response.data.result.id,type: 'create',module: 'Meetings',lead_id: this.state.parent_id, parent_name: this.state.parent_name}, // simple serializable object that will pass as props to all top screens (optional)
                    animationType: 'slide-horizontal'
                })
                ...
                apiCallForToken.getToken(this.props).then(token => {
                    ...
                    //update the meeting, after session generated
                    this.updateMeeting()
                ...
                Alert.alert('Error', response.data.description===undefined?response.message : response.data.description);   
                this.setState({ loading: false });
                this.setState({ disable: false });
            }
...
/**
    ...
    setReminder(response) {
        this.notif.updateScheduledNotification(response, globals.meetings, globals.leads)
    }


    //relate this meeting to the opportunity
    relateMeeting(id) {

        let url = globals.home_url + "/setRelationship" + "?token_id=" + this.props.token + "&module_name=" + globals.leads +
            "&module_id=" + this.state.parent_id + "&link_field_name=meetings" + "&related_ids=" + id 
            + "&delete=0"
            + "&url="+this.props.url;
        ...
        fetch(url, {
            ...
            .then((response) => {
                ...
                if (response.success === true) {
                    ...
                    //                 screen: {
                    //                     screen: 'app.MeetingDetailsOpportunity',
                    //                     title: '',
                    //                     animationType: 'slide-horizontal',

                    //                 },
                    //                 passProps: { id: id, type: 'create', module: 'Meetings', opportunity_id: this.state.adata.id, parent_name: parent_name }, // simple serializable object that will pass as props to all top screens (optional)
                    //                 animationType: 'slide-horizontal'
                    //             })
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(UpdateMeetingLead);