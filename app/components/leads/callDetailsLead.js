...
import { Container, Content, Label, Item, Button, Text } from 'native-base';
...
class CallDetailsLeads extends Component {
        ...
        if (event.type == 'NavBarButtonPress') {
            ...
            if (event.id == 'edit') { // this is the same id field from the static navigatorButtons definition
                ...
                this.props.navigator.push({
                    screen: 'app.UpdateCallLead',
                    title: 'Edit Call',
                    subtitle: undefined, // navigation bar subtitle of the pushed screen (optional)
                    passProps: { data: this.state.call_data, lead_id: this.props.leads_id , parent_name: parent_name}, // Object that will be passed as props to the pushed screen (optional)
        ...
        fetch(url, {
            ...
            .then((response) => {
                ...
                if (response.success === true) {
                    ...
                    console.log('before ', this.state.call_data.date_start);
                    this.setState({
                        date_formatted: globals.formatUtcDateAndTimeToLocal(this.state.call_data.date_start)
                    });

                    console.log('after ', this.state.date_formatted);
                    if (this.props.from_list) {
                        this.props.navigator.push({
                            screen: 'app.UpdateCallLead',
                            title: 'Edit Call',
                            subtitle: undefined, // navigation bar subtitle of the pushed screen (optional)
                            passProps: { data: this.state.call_data, lead_id: this.props.leads_id, parent_name: response.data.result.parent_name }, // Object that will be passed as props to the pushed screen (optional)
                            animated: true, // does the push have transition animation or does it happen immediately (optional)
                            animationType: 'slide-horizontal', // 'fade' (for both) / 'slide-horizontal' (for android) does the push have different transition animation (optional)
                            backButtonTitle: 'Back' // override the back button title (optional)
                        });
...
export default Login = connect(mapStateToProps, mapDispatchToProps)(CallDetailsLeads);